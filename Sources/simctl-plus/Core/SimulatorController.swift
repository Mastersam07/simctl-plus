...
struct SimulatorDiagnostics {
    ...
    let errors: [String]
    let warnings: [String]
    let performance: [String: TimeInterval]
}
...
class SimulatorController {
    ...
    func analyzeLogs(deviceId: String) throws -> SimulatorDiagnostics {
        let logOutput = try getDiagnostics(deviceId: deviceId)
    ...
    func measureStartupTime(deviceId: String) async throws -> TimeInterval {
        ...
        while !isBooted {
            ...
            if let simulator = simulators.first(where: { $0.udid == deviceId }) {
